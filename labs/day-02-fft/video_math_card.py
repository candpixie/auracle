...
def main():
    ...
    fig.text(0.5, 0.93, "this isn't a bug.", ha="center", va="top",
             **display(44), color=FG, weight="bold")

    fig.text(0.5, 0.80, "time resolution", ha="center", **text(27), color=DIM)
    fig.text(0.5, 0.755, "×", ha="center", **display(34), color=ACCENT)
    fig.text(0.5, 0.715, "frequency resolution", ha="center", **text(27), color=DIM)
    fig.text(0.5, 0.645, "= 1", ha="center", **display(68), color=ACCENT, weight="bold")
    fig.text(0.5, 0.585, "always.", ha="center", **display(31), color=FG)
    ...
    y = 0.500
    fig.text(0.15, y, "chunk", **text(22), color=DIM)
    fig.text(0.52, y, "knows WHEN", **text(22), color=DIM, ha="center")
    fig.text(0.85, y, "knows WHAT", **text(22), color=DIM, ha="right")
    fig.add_artist(plt.Line2D([0.13, 0.87], [y - 0.020, y - 0.020],
                              color="#3a3350", lw=2))

    for i, n in enumerate(WINDOWS):
        row = y - 0.072 - i * 0.058
        fig.text(0.15, row, f"{n} samples", **text(27), color=FG, va="center")
        fig.text(0.52, row, f"{n / FS * 1000:.1f} ms", **text(28), color=FG,
                 ha="center", va="center")
        fig.text(0.85, row, f"{FS / n:.0f} Hz", **text(28), color=FG,
                 ha="right", va="center")

    fig.text(0.5, 0.245,
             "make one better\nand the other gets worse.\nexactly as much.",
             ha="center", va="top", **display(30), color=FG, linespacing=1.5)

    fig.text(0.5, 0.075, "it's algebra.", ha="center", **display(33), color=ACCENT,
             weight="bold")
    fig.text(0.5, 0.030, "nobody is fixing this.", ha="center", **display(33),
